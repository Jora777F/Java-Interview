...
Ниже представлены вопросы по Java.
Получилось 49 вопросов. В каждом вопросе представлено несколько
вариантов ответов.

Надеюсь, что этот материал будет кому-нибудь полезен.
...
## Вопрос №6.
Приложение работает в Kubernetes, метрики собираются Prometheus.
На графике видно рост задержки запросов при увеличении GC-пауз.
Какое действие наиболее правильное?
1) Увеличить heap JVM до максимума, чтобы уменьшить GC частоту
2) Включить больше pod-ов, GC на каждом уменьшится автоматически
3) Игнорировать GC, так как Prometheus показывает только системные метрики
4) Настроить метрики GC (Pause Time, Allocation Rate) и проанализировать параметры JVM GC (например, G1GC tuning).
...
## Вопрос №7.
При использовании Spring Data JPA метод репозитория:
List<User> findByEmail(String email);
возвращает несколько записей, хотя email уникален. Какое объяснение
верное?
1) В БД отсутствует уникальный индекс на колонку email
2) Spring Data не накладывает ограничений на уникальность полей
3) При запросе используется Criteria API вместо SQL
4) Метод не поддерживает поиск по уникальным полям
...
## Вопрос №8.
При проектировании системы было решено, что объект
Invoice должен сам вычислять сумму по позициям, а не сервис "снаружи".
Какой принцип GRASP применяется?

1) Low Coupling - снижение связности модулей
2) Controller - объект управляется сценарием
3) Creator - объект создает зависимые объекты
4) Information Expert - ответственность возложена на того, у кого есть
данные
...
## Вопрос №33.
![Question](assets/33-question.png)
...
## Вопрос №34.
![Question](assets/34-question.png)
...
## Вопрос №35
![Question](assets/35-question.png)
...
## Вопрос №36.
![Question](assets/36-question.png)
...
## Вопрос №37.
![Question](assets/37-question.png)
...
## Вопрос №38.
![Question](assets/38-question.png)
...
## Вопрос №39.
![Question](assets/39-question.png)
...
## Вопрос №40.
![Question](assets/40-question.png)
...
## Вопрос 41.
![Question](assets/41-question.png)
